...
status: notes
sessions: [2026-04-14, 2026-04-15, 2026-04-16, 2026-04-17]
---
...
### Nameservers must match where your DNS records live [`debugging`]

DNS records weren't working even though they were configured correctly in Porkbun. The root cause: the domain's nameservers weren't pointing to Porkbun's default nameservers, so the DNS records had nowhere to resolve from. `nslookup` returned "Non-existent domain" — not a propagation delay, but a complete failure. The fix was switching to Porkbun's default nameservers in the domain management dashboard. Lesson: DNS records only work if the nameservers are set to the provider where those records are configured.

### SSL certificates are auto-generated by Vercel [`concept`]

After DNS pointed to Vercel correctly, Vercel automatically generated an SSL certificate for the custom domain. This enables HTTPS — no manual certificate setup needed. The status in the Vercel dashboard changed from "Invalid Configuration" to "Generating SSL Certificate" to fully verified. This is a feature of modern hosting platforms; on traditional servers, you'd need to set up SSL yourself (e.g., with Let's Encrypt).
